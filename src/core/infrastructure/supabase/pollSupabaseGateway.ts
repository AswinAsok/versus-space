...
export const pollFacade = {
  async createPoll(data: CreatePollData, userId: string) {
    ...
    const { data: poll, error: pollError } = await supabase
      ...
      .insert({
        ...
        auto_vote_interval_seconds:
          safeData.auto_vote_interval_seconds ?? AUTO_VOTE_INTERVAL_DEFAULT_MS,
      })
      .select()
      .single();
  ...
  async getUserPolls(userId: string) {
    ...
    const optionsByPollId = (options || []).reduce(
      (acc, opt) => {
  ...
  async updatePoll(pollId: string, data: UpdatePollData) {
    ...
    if (data.auto_vote_interval_seconds !== undefined)
      pollUpdate.auto_vote_interval_seconds = normalizeAutoVoteIntervalMs(
        data.auto_vote_interval_seconds
      );

    if (Object.keys(pollUpdate).length > 0) {
      const { error: pollError } = await supabase.from('polls').update(pollUpdate).eq('id', pollId);
      if (pollError) throw pollError;
    }
    ...
    const updatedPoll = await pollFacade.getPoll(pollId);
