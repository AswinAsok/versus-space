...
interface VoteAuthenticityStats {
  realVotes: number;
...
export const voteFacade = {
  ...
  async updateUserSession(userId: string, pollId: string) {
  ...
  async getVotesOverTime(pollIds: string[], days: number) {
    ...
    const { data, error } = await supabase.rpc('get_votes_over_time', {
      p_poll_ids: pollIds,
    ...
    pollIds.forEach((pollId) => {
      votesByPoll.set(pollId, new Map(bucketKeys.map((key) => [key, 0])));
    });
  ...
  async getVoteTimestamps(pollIds: string[], days: number) {
    ...
    const { data, error } = await supabase.rpc('get_vote_counts_by_date', {
      p_poll_ids: pollIds,
      p_start_date: startDate.toISOString(),
    });
