...
export function VoteToast({ pollIds, pollTitles }: VoteToastProps) {
  ...
  const addToast = useCallback(
    (optionTitle: string, pollTitle: string, isSimulated: boolean) => {
      ...
      setToasts((prev) => {
        ...
        if (existing) {
          ...
          return prev.map((t) =>
            `${t.optionTitle}-${t.pollTitle}` === key ? { ...t, count: t.count + 1 } : t
          );
        } else {
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtimeFacade.subscribeToPolls(pollIds, {
      ...
      onVote: (vote) =>
        addToast(vote.optionTitle, pollTitles.get(vote.pollId) || 'Unknown', vote.isSimulated),
    });
